...
try:
    f = open('robot.setting', 'r')
    ipaddr = f.readline()
    port = int(f.readline())
    node = f.readline()[:-1]
    node = node if not node else '#'+node
except:
    print("Setting")
    f = open('robot.setting', 'w')
    f.write(input("IP Address: ")+'\n')
    f.write(input("Port: ")+'\n')
    f.write(input("Node Number: ")+'\n')
    f.close()
    sys.exit()


vrep.simxFinish(-1)
clientID=vrep.simxStart(ipaddr,port,True,True,5000,5) # Connect to V-REP
...
workbook = xlsxwriter.Workbook('data-robot'+node+'.xlsx')
